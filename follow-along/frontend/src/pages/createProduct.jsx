...
function CreateProduct() {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const response = await axios.put(`http://localhost:4534/product/update/${_id}`, multiPartFormData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  return (
    <div
      ...
      style={{ backgroundImage: "url('https://source.unsplash.com/1600x900/?office,technology')" }}
    >
      <div className="w-full max-w-lg bg-white p-6 rounded-lg shadow-lg backdrop-blur-md bg-opacity-90">
        ...
        <form className="space-y-4">
          ...
          <label
            htmlFor="upload"
            className="cursor-pointer flex items-center space-x-2 text-blue-500 hover:text-blue-700"
          >
            ...
            {formData.previewImg.map((img, index) => (
              <div key={index}>
                <IoCloseCircleOutline
                  onClick={() => handleDeletePrevImg(index)}
